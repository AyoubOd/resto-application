    ...
    <div class="py-12 w-1/2 mx-auto">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form class="bg-gray-100 rounded-lg px-6 py-4"
                action="{{ route('admin.tables.update', ['table' => $table]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <select name="status"
                        ...
                        @foreach (App\Enums\TableStatus::cases() as $status)
                            <option value="{{ $status->value }}" @selected(old('status', $table->status->value) == $status->value)>
                                {{ $status->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="location"
                        ...
                        @foreach (App\Enums\TableLocation::cases() as $location)
                            <option value="{{ $location->value }}" @selected(old('location', $table->location->value) == $location->value)>
